...
class ApplicationController < ActionController::Base
  ...
  def open_graph_title_and_desc(api_gift)
    ...
    if text.length <= title_lng
      ...
      og_desc_key = "gifts.show.og_desc_#{api_gift.provider}"
      begin
        description = t og_def_desc_key, :appname => APP_NAME, :api_name => provider_downcase(api_gift.provider), :raise => I18n::MissingTranslationData
