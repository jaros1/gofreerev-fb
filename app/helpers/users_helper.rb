module UsersHelper
  ...
  def gift_balance_calculation(gift)
    ...
    translate_key << 'negative_interest' if negative_interest.round(2) != 0.0
    translate_key = translate_key.join('_')
    price = gift.price
    my_t translate_key, :old_balance => format_price(balance_doc[:previous_balance]),
                        :new_balance => format_price(gift.balance(@user.user_id)),
